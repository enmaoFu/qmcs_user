...
public class MineTransferOutTimeDetailAty extends BaseAty {
    ...
    public void requestData() {
        ...
        doHttp(RetrofitUtils.createApi(Transfer.class).excepPackageInfo(1, getIntent().getLongExtra("orderId", 0)), 1);
    ...
    private PermissionListener listener = new PermissionListener() {
        ...
        public void onSucceed(int requestCode, List<String> grantedPermissions) {
            ...
            if (requestCode == 200) {
                ...
                if (type == 0) {
                    new MaterialDialog(MineTransferOutTimeDetailAty.this)
    ...
    public void onSuccess(String result, Call<ResponseBody> call, Response<ResponseBody> response, int what) {
        ...
        switch (what) {
            case 1:
                MineTransferOutTimeDetial detailResult = AppJsonUtil.getObject(result, MineTransferOutTimeDetial.class);
                ...
                String startTime = DateTool.getTimeType(detailResult.getReqPickupTime());
                String endTime = DateTool.getTimeType(detailResult.getReqDelivTime());

                if (startTime == null) {
                    mTvStartTime.setText(DateTool.timesToStrTime(detailResult.getReqPickupTime() + "", "yyyy.MM.dd HH:mm"));
                } else {
                    mTvStartTime.setText(startTime + DateTool.timesToStrTime(detailResult.getReqPickupTime() + "", "HH:mm") + "之前");
                }

                if (endTime == null) {
                    mTvEndTime.setText(DateTool.timesToStrTime(detailResult.getReqDelivTime() + "", "yyyy.MM.dd HH:mm"));
                } else {
                    mTvEndTime.setText(endTime + DateTool.timesToStrTime(detailResult.getReqDelivTime() + "", "HH:mm") + "之前");
                }
                //
                ...
                //判断是否已送达
                if (detailResult.getCheckTime() != 0) {
                    mTvSendTime.setText("送达时间：" + DateTool.timesToStrTime(detailResult.getCheckTime() + "", "yyyy.MM.dd HH:mm"));
                    mTvOutTime.setText(getTime((detailResult.getCheckTime() - detailResult.getReqDelivTime()) / 1000));
                } else {
                    mTvSendTime.setText("送达时间：未送达");
                    mTvOutTime.setText(getTime((System.currentTimeMillis() - detailResult.getReqDelivTime()) / 1000));
                }


                mTvGetTime.setText("接单时间：" + DateTool.timesToStrTime(detailResult.getPickupTime() + "", "yyyy.MM.dd HH:mm"));
